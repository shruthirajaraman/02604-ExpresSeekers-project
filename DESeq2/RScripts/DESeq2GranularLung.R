...
# Set up
normal_condition <- "na_normal_sample"
...
label_map <- c(
  ...
  acinar_cell_neoplasms = "Acinar Cell Neoplasms",
  cystic_mucinous_and_serous_neoplasms = "Cystic, Mucinous & Serous Neoplasms"
)
...
for (cancer_type in cancer_conditions) {
  ...
  if (sum(subset_meta$condition == cancer_type) < 4) {
  ...
  out_dir <- paste0("results/", cancer_type, "_vs_normal/")
  ...
  gg_ma <- ggplot(res_annotated, aes(x = log10(mean + 1), y = log2FoldChange)) +
    ...
    ylim(-5, 5) +
    labs(title = paste("MA Plot:", plot_label, "vs Normal"),
...
write.csv(summary_df, file = "results/DEG_summary_table.csv", row.names = FALSE)
